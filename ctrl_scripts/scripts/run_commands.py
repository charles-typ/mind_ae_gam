...
def build_first_access_vm_command(server_ip, s_user, s_key, vm_ctrl_ip, v_user, v_key):
    inner_cmd = build_ssh_base(vm_ctrl_ip, v_user,
                               v_key) + " -o \'StrictHostKeyChecking no\' -o ConnectTimeout=10 -t \'exit\'"
...
def build_setup_compute_server_command(server_ip, user, key, idx):
    ...
    cmd = build_ssh_base(server_ip, user, key)
    cmd += ' -t \"cd /local/repository/ && echo Y | ./init_env.sh && ./localize_data.sh CN ' + str(idx) + ' ' + str(
        idx) + '\"'
    return cmd
...
def build_vm_cn_create_command(server_ip, user, key, idx, system_name="mind"):
    ...
    cmd = build_ssh_base(server_ip, user, key)
    cmd += ' -t \"cd /local/repository/ && echo Y | ./setup_CN.sh ' + system_name + ' ' + str(idx + 1) + ' ' + str(
        idx + 1) + '\"'
    return cmd
...
def build_vm_init_mn_gam_command(server_ip, s_user, s_key, vm_ctrl_ip, v_user, v_key, script_dir, controller_ip, worker_ip,
                                 controller_port, worker_port):
    return build_vm_brick_command(server_ip, s_user, s_key,
                                  vm_ctrl_ip, v_user, v_key,
                                  script_dir, "v_init_mn_module_gam.sh "
                                  + str(controller_ip) + " " + str(worker_ip) + " " + str(controller_port) + " " + str(worker_port))
...
def run_on_all_vms(cfg, job="dummy", job_args=None, verbose=True, per_command_delay=1, post_delay=1,
                   target_vms=[]):  # 1 sec of delay by default
            ...
            for server in cfg[key_cs]:
                ...
                if job == "first_access":
                        ...
                        for storage_server in cfg[key_ss]:
                            ...
                            for app_names in app_name_map:
                                ...
                                for trace_slice in [key_dir_1, key_dir_2]:
                                    delete_cmd += "echo " + delete_dst[trace_slice] + " && sudo mkdir -p " + delete_dst[
                                        trace_slice]
                            ...
                            cmd = build_host_load_trace_command(server[key_ip], s_user_id, s_ssh_key,
                                                                script_root, app_name_map[job_args[key_trace]],
                                                                trace_src,
                                                                trace_dst[key_dir_1], trace_dst[key_dir_2], s_user_id,
                                                                storage_server[key_cluster_ip], str(server[key_id]),
                                                                delete_cmd, storage_server[key_ssh_key])
                elif job == "perm_trace":
                    ...
                    if (key_nic in server) and (key_cluster_ip in server) \
                        ...
                        cmd += " ; sudo arp -s -i " + server[key_nic] + " " + cfg[key_ss][0][key_cluster_ip] + " " + \
                               cfg[key_ss][0][key_mac]
                ...
                for vm in server[key_vm]:
                    ...
                    if job == "first_access":
                        ...
                        if (job_args is not None) and (key_system in job_args):
                            cmd = build_vm_cn_create_command(server[key_ip], s_user_id, s_ssh_key, vm[key_id],
                                                             job_args[key_system])
                    elif job == "reset":
                        ...
                        if (job_args is not None) and (key_state_from in job_args) \
                            ...
                            if key_main_vm in job_args and job_args[key_main_vm] == vm[key_id]:
                                cmd = build_vm_latency_run_command(server[key_ip], s_user_id, s_ssh_key,
                                                                   vm[key_ip], v_user_id, v_ssh_key, script_root,
                                                                   vm[key_id], job_args[key_state_from],
                                                                   job_args[key_state_to],
                                                                   job_args[key_node_num])
                    ...
                    if cmd2 is not None:
                        ...
                        tasks.append(loop.create_task(
                            run_command_sync_print(
                                loop, cmd2, (collect_std_out and verbose), async_delay)))
                        async_delay += per_command_delay
        ...
        for server in cfg[key_cs]:
            for vm in server[key_vm]:
                if (key_id in vm) and (key_mac in vm) and (key_cluster_ip in vm):
                    compute_mac_list.append(
                        {key_id: vm[key_id], key_mac: vm[key_mac], key_cluster_ip: vm[key_cluster_ip]})
...
if __name__ == "__main__":
    ...
    parser.add_argument('--profile', type=str,
                        help='Relative path to the profile of the test/benchmark you want to run', default="")
